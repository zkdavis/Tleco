...
def run_katarzynski():
    ...
    n8[0, :] = misc_func.power_law(7, g, p, gcut8, g2cut8)
    ...
    progress_bar = tqdm(total=max_numt - 1, desc='Progress')
    # Time loop
    for i in range(1, max_numt):
        if(i<numt):
            ...
            n4[i, :] = para.fp_findif_difu(dt, g, n4[i - 1, :], gdot4[i - 1, :], D4, (2e4)*n4[0, :]/t_esc, t_esc, tlc, False)
            n5[i, :] = para.fp_findif_difu(dt, g, n5[i - 1, :], gdot5[i - 1, :], D5, (2e4)*n5[0, :]/t_esc, t_esc, tlc, False)
            n6[i, :] = para.fp_findif_difu(dt, g, n6[i - 1, :], gdot6[i - 1, :], D6, (2e4)*n6[0, :]/t_esc, t_esc, tlc, False)
        ...
        if (i < numt3):
            ...
            # dotgKN8 = para.rad_cool_pwl(g, f, 4 * np.pi * I_s7[i, :] / cons.cLight, cool_withKN)
...
def n_plot(n, g,compare_fig=None):
    ...
    if compare_fig:
        if (compare_fig == 'fig1_a'):
            ...
            t_acc_temp = t_acc
            x_data, y_data = get_data("compare_data/katarzynski_digitized_plots/fig1_a")
            argsort = x_data.argsort()
            ...
            t_times_temp = np.array([0.01, 0.1, 0.5, 1, 2, 3, 5, 10,15,40]) * t_acc
            t_acc_temp = t_acc
            x_data, y_data = get_data("compare_data/katarzynski_digitized_plots/fig1_b")
            argsort = x_data.argsort()
            x_data = x_data[argsort]
            y_data = y_data[argsort]
            ...
            t_times_temp = np.array([0.01, 0.1, 0.5, 1, 2, 3, 5, 10,15,40]) * t_acc
            t_acc_temp = t_acc
            x_data, y_data = get_data("compare_data/katarzynski_digitized_plots/fig1_c")
            argsort = x_data.argsort()
            x_data = x_data[argsort]
            y_data = y_data[argsort]
            ...
            t_acc_temp = t_acc
            x_data, y_data = get_data("compare_data/katarzynski_digitized_plots/fig2_a")
            argsort = x_data.argsort()
            x_data = x_data[argsort]
            y_data = y_data[argsort]
            ...
            t_times_temp = np.array([0.01, 0.1, 0.5, 1, 2, 3, 5, 10]) * t_acc
            t_acc_temp = t_acc
            x_data, y_data = get_data("compare_data/katarzynski_digitized_plots/fig2_b")
            argsort = x_data.argsort()
            x_data = x_data[argsort]
            y_data = y_data[argsort]
            ...
            t_times_temp = np.array([0.01, 0.1, 0.5, 1, 2, 3, 5, 10]) * t_acc
            t_acc_temp = t_acc
            x_data, y_data = get_data("compare_data/katarzynski_digitized_plots/fig2_c")
            argsort = x_data.argsort()
            x_data = x_data[argsort]
            y_data = y_data[argsort]
            ax.set_ylim(1e-8, 1e7)
            ...
            t_acc_temp = t_acc7
            x_data, y_data = get_data("compare_data/katarzynski_digitized_plots/fig3_a")
            argsort = x_data.argsort()
            x_data = x_data[argsort]
            y_data = y_data[argsort]
            ...
            t_acc_temp = t_acc8
            x_data, y_data = get_data("compare_data/katarzynski_digitized_plots/fig4_a")
            argsort = x_data.argsort()
            x_data = x_data[argsort]
            y_data = y_data[argsort]
    ...
    if (x_data is not None):
        ax.plot(x_data, y_data, color='k', lw=3, linestyle='--')
...
def nuFnu_plots(Im, nu, compare_fig):
    ...
    if compare_fig == 'fig3_b':
        ...
        t_acc_temp = t_acc7
        x_data, y_data = get_data("compare_data/katarzynski_digitized_plots/fig3_b")
        argsort = x_data.argsort()
        x_data = x_data[argsort]
        y_data = y_data[argsort]
        d_temp = 4.32e26
        ...
        t_times_temp = np.array([2.5,3.5,4.0, 4.5, 5, 7]) * t_acc7
        t_acc_temp = t_acc7
        x_data, y_data = get_data("compare_data/katarzynski_digitized_plots/fig3_c")
        argsort = x_data.argsort()
        x_data = x_data[argsort]
        y_data = y_data[argsort]
        R_temp = R
        ...
        t_acc_temp = t_acc8
        x_data, y_data = get_data("compare_data/katarzynski_digitized_plots/fig4_b")
        argsort = x_data.argsort()
        x_data = x_data[argsort]
        y_data = y_data[argsort]
        d_temp = 4.32e26
        ...
        t_acc_temp = t_acc8
        x_data, y_data = get_data("compare_data/katarzynski_digitized_plots/fig4_c")
        argsort = x_data.argsort()
        x_data = x_data[argsort]
        y_data = y_data[argsort]
        d_temp = 4.32e26

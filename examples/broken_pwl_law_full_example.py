...
for i in range(1, len(t)):
    dt = t[i] - t[i - 1]
    n[i, :] = para.fp_findif_difu(
        dt, g, n[i - 1, :], gdot[i - 1, :], D, Qinj, 1e200, tlc
    )
...
def plot_generic(
    ...
    sm = plt.cm.ScalarMappable(
        cmap=cmap, norm=matplotlib.colors.LogNorm(vmin=t[0], vmax=t[-1])
    )
